...
Most users never set these; the command-line flags feed the four settings below — `--host`, `--port`, and `--trusted-host` come from the invocation, `--no-open` turns the browser handoff off for that invocation, and `--base-path` mounts the whole surface under a sub-path (see [HTTP server](../../host/webserver/README.md)):
...
| [`src/startup.ts`](src/startup.ts) | The `web-startup` provider: `--host`, `--port`, `--trusted-host`, `--base-path`, `--no-open`, `--help` |
